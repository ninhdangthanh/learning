...
| **Token Bucket** | Hash `{tokens, updated_at}`, refill theo thời gian trôi | `POST /auth/login`, `POST /auth/refresh`, `POST/PATCH/DELETE /api/notes` (bucket chung) | burst 10 (auth) · burst 20 (write) | IP · user |
...
* **Login** cần cho người dùng gõ sai vài lần liên tiếp mà không bị chặn ngay, rồi siết dần. Đó đúng là hành vi token bucket: cho burst, sau đó chỉ nhả token từ từ. Các thao tác ghi `POST/PATCH/DELETE /api/notes` dùng lại ý tưởng này cho thao tác ghi — cho phép nhập liệu dồn dập trong chốc lát, nhưng không cho spam kéo dài.
...
| `PATCH` | `/api/notes/{id}` | token bucket | Sửa title và/hoặc body |
| `DELETE` | `/api/notes/{id}` | token bucket | Xoá |
